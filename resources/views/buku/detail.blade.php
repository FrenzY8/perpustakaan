...
<body class="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white min-h-screen">
      ...
      <div class="lg:col-span-7 xl:col-span-8 space-y-8">
        <div class="space-y-4">
          ...
          <div class="space-y-1">
            ...
            <a href="/buku?search={{ urlencode($book->penulis->nama ?? '') }}&sort=latest"
              class="text-xl text-primary font-medium italic hover:underline">
              by {{ $book->penulis->nama ?? 'Penulis tidak diketahui' }}
            </a>
          </div>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div class="glass-panel p-4 rounded-xl flex flex-col gap-2">
            ...
            <div>
              ...
              <p class="text-white/40 text-xs uppercase tracking-wider font-semibold">JUMLAH HALAMAN</p>
            </div>
          </div>
          <div class="glass-panel p-4 rounded-xl flex flex-col gap-2">
          ...
          <div class="glass-panel p-4 rounded-xl flex flex-col gap-2">
            ...
            <div>
              <p class="text-white text-base font-bold">
                {{ $book->kategori->nama ?? 'Kategori tidak diketahui' }}
              </p>
              <p class="text-white/40 text-xs uppercase tracking-wider font-semibold">Kategori</p>
            </div>
          </div>
          <div class="glass-panel p-4 rounded-xl flex flex-col gap-2">
          ...
          <div class="glass-panel p-4 rounded-xl flex flex-col gap-2">
            <span class="material-symbols-outlined text-primary">window</span>
            <div>
              <p class="text-white text-base font-bold">
                {{ $book->stok }}
              </p>
              <p class="text-white/40 text-xs uppercase tracking-wider font-semibold">STOK TERSEDIA</p>
            </div>
          </div>
        </div>
  ...
  : 'glass-panel border-white/10 hover:bg-white/10 text-white' 
        ...
        <div class="border-t border-white/5 pt-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
          ...
          <div class="flex flex-col gap-1">
            <span class="text-white/30 uppercase tracking-widest font-bold text-[10px]">Tags</span>
            <div class="flex flex-wrap gap-1.5 mt-1">
              @forelse($book->tags as $tag)
                <span class="px-2 py-0.5 bg-white/5 border border-white/10 rounded-md text-[11px] text-white/60">
                  #{{ $tag->nama }}
                </span>
              @empty
                <span class="text-white/20 italic text-[11px]">Tidak ada tag</span>
              @endforelse
            </div>
          </div>
